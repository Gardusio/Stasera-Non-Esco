...
namespace StaseraNonEsco.Services
{
    public static class ApiService
    {
        ...
        public static async Task<bool> RegisterAdmin(string name, string email, string password) {
            var register = new Register() {
                Name = name,
                Email = email,
                Password = password
            ...
            var json = JsonConvert.SerializeObject(register);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await httpClient.PostAsync(AppSettings.ApiUrl + "api/Accounts/RegisterAdmin", content);
            if (!response.IsSuccessStatusCode) return false;
            return true;
        }
